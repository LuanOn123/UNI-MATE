...
## SD-02 — Đăng nhập và xác thực hai bước

```mermaid
sequenceDiagram
    autonumber
    actor U as Người dùng hoặc Admin
    participant UI as AuthPage/OtpPage «boundary»
    participant AC as AuthController «control»
    participant AS as AuthService «control»
    participant UE as User «entity»
    ...
    alt Không có user hoặc sai mật khẩu
        AS-->>UI: 401 Invalid credentials
    else User bị suspended, banned hoặc inactive
        AS-->>UI: 403 Account is locked
    else Mật khẩu đúng
        alt role=admin hoặc twoFactorEnabled=true
            AS->>AS: Sinh và hash OTP
            AS->>OE: create(email, otpHash, expiresAt)
            AS->>MAIL: sendOtpEmail(email, otp)
            MAIL-->>U: Email chứa OTP
            AS-->>UI: requiresTwoFactor=true
            UI-->>U: Mở OtpPage
            U->>UI: Nhập OTP
            UI->>AC: verifyOtp(email, otp)
            AC->>AS: verifyEmailOtp(email, otp)
            AS->>OE: Tìm OTP mới nhất chưa dùng, chưa hết hạn
            alt OTP sai hoặc vượt số lần thử
                AS->>OE: Tăng attempts
                AS-->>UI: 400 hoặc 429
            else OTP đúng
                AS->>OE: consumed=true
                AS->>UE: Cập nhật emailVerified, lastLoginAt, lastSeenAt
                AS->>TS: issueTokens(user)
                TS-->>AS: accessToken, refreshToken
                AS-->>UI: User và tokens
            end
        else Không yêu cầu 2FA
