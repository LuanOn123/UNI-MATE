...
export const adminUsers = asyncHandler(async (req, res) => {
    ...
    if (req.query.q)
        query.$or = [{ email: new RegExp(String(req.query.q), "i") }, { displayName: new RegExp(String(req.query.q), "i") }];
...
export const updateUserStatus = asyncHandler(async (req, res) => {
    ...
    const statusUpdate = { $set: { status, isActive: status === "active" } };
    if (status !== "suspended")
        statusUpdate.$unset = { suspendedUntil: 1 };
...
export const adminReportDetail = asyncHandler(async (req, res) => {
    ...
    const messages = report.room
        ? await Message.find({ room: report.room._id ?? report.room }).sort({ createdAt: -1 }).limit(200).populate("sender", "displayName avatarUrl email")
        : [];
...
export const updateReport = asyncHandler(async (req, res) => {
    ...
    if (action === "dismiss") {
        ...
        if (suspendDays !== undefined && ![3, 5, 7].includes(Number(suspendDays)))
            return res.status(400).json({ message: "Số ngày tạm khóa chỉ được chọn 3, 5 hoặc 7 ngày" });
        suspensionEnd = suspendedUntil ? new Date(suspendedUntil) : new Date(Date.now() + Number(suspendDays || 3) * 24 * 60 * 60 * 1000);
        if (!Number.isFinite(suspensionEnd.getTime()) || suspensionEnd.getTime() <= Date.now())
            return res.status(400).json({ message: "Ngày kết thúc tạm khóa phải ở tương lai" });
        user.status = "suspended";
        user.isActive = false;
        user.suspendedUntil = suspensionEnd;
        await user.save();
    ...
    if (user.status === "suspended" || user.status === "banned")
        io?.in(`user:${String(user._id)}`).disconnectSockets(true);
...
export const adminPlaces = asyncHandler(async (req, res) => {
    ...
    if (req.query.district)
        query.district = new RegExp(String(req.query.district), "i");
    if (req.query.q) {
        query.$or = [
...
export const adminTags = asyncHandler(async (_req, res) => {
    res.json({ tags: await Tag.find().sort({ type: 1, name: 1 }) });
});
export const upsertTag = asyncHandler(async (req, res) => {
...
export const adminActions = asyncHandler(async (_req, res) => {
