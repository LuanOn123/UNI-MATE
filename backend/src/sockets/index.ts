...
export function registerSockets(io: Server) {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("send_message", async (data: { roomId: string; text: string; type?: string; fileUrl?: string; fileName?: string }) => {
      try {
        const room = await ChatRoom.findById(data.roomId);
        if (!room || room.status !== "active") return socket.emit("message_error", { roomId: data.roomId, message: "Chat is not active" });
        
        if (!data.text?.trim() && !data.fileUrl) return socket.emit("message_error", { roomId: data.roomId, message: "Tin nhắn không được để trống" });
        
        const match = await Match.findById(room.match);
        if (match?.status !== "chat_opened") return socket.emit("message_error", { roomId: data.roomId, message: "Chat is not active" });
    ...
    socket.on("send_group_message", async (data: { groupId: string; text: string; type?: string; fileUrl?: string; fileName?: string }) => {
      try {
