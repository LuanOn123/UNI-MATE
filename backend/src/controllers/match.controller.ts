...
async function openChatForMatch(match: any) {
  if (!match || ["expired", "blocked", "cancelled"].includes(match.status)) return match;
  const users = (match.users ?? []).map((user: any) => user?._id ?? user);
  const room = await ChatRoom.findOneAndUpdate(
    { match: match._id },
    { match: match._id, users, status: "active" },
